    ...
    if(_.isArray(json)){
        ...
        parseResult.forEach(function (result) {
            ...
            result.forEach(function(element){
                ...
                var value = element.value ? element.value.toString() : options.undefinedString;
                value = escapeString(value);
                resultRows.forEach(function (row) {
                    if(!placed && row[headers.indexOf(element.item)] == ''){
                        row[headers.indexOf(element.item)] = value;
                        placed = true;
                    }
                });
                ...
                if(!placed){
                    ...
                    newRow[headers.indexOf(element.item)] = value;
...
function checkType(element, item){
    ...
    // jclay: hack to catch any functions, reject them and don't process
...
function escapeString(string) {
    ...
    */
    ...
    string = string.replace(textDelimiterRegex, options.textDelimiter+options.textDelimiter);
    // Escape the whole field if it contains a rowDelimiter or a linebreak
    if (string.indexOf(options.rowDelimiter) >= 0 || string.indexOf(endOfLine) >= 0) {
        string = options.textDelimiter + string + options.textDelimiter;
    }
    return string;
}
